...
describe('Testing App() Component', () => {
  ...
  it('should decrement only the selected counter', () => {
    const state = [
      { id: 1, value: 2 },
      { id: 2, value: 3 },
    ]
    render(<App state={state} />)
    const toDecrement = screen.queryAllByTestId('decrement-button')
    fireEvent.click(toDecrement[1])
    const counters = screen.queryAllByTestId('total-counter')
    expect(counters[0]).toHaveTextContent('2')
    expect(counters[1]).toHaveTextContent('2')
  })
  ...
  it('should delete', () => {
    ...
    expect(size).toBeGreaterThan(newSize)
  })
